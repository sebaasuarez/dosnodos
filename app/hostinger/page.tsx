...
import { NECESIDADES, PREGUNTAS, PRODUCTOS, enlaceProducto } from "@/lib/hostinger"
import { SeccionHostinger } from "@/components/hostinger/seccion"
import { NavHostinger } from "@/components/hostinger/nav"
import { PieHostinger } from "@/components/hostinger/pie"
import { Faq } from "@/components/hostinger/faq"
import { DatosHostinger } from "@/components/hostinger/datos"
import {
  SeccionProblema,
  SeccionSoluciones,
  SeccionReparto,
  SeccionProceso,
  SeccionPreguntas,
} from "@/components/hostinger/secciones"
...
export default async function HostingerPage() {
  ...
  const correo = settings.contact_email || "hola@dosnodos.com.co"
  const whatsapp = settings.whatsapp_number || "573127344026"
  ...
  return (
    <>
      ...
      <main className="bg-white">
        ...
        <section className="border-b border-[#EDEAF6] bg-[#F9F8FD] px-6 py-16 md:py-24">
          <div className="mx-auto flex max-w-[1180px] flex-col gap-6">
            ...
            <p className="max-w-[58ch] text-[clamp(1rem,1.6vw,1.15rem)] leading-[1.55] text-[#5A5570]">
              Te ayudamos a elegir la solución adecuada y nos encargamos de configurarla, conectarla
              y convertirla en una plataforma lista para tu negocio.
            </p>
        ...
        >
          <SeccionProblema />
          <SeccionSoluciones referral={referral} />
          <SeccionReparto />
          <SeccionProceso />
          <SeccionPreguntas>
      ...
      <PieHostinger contactEmail={correo} whatsapp={whatsapp} />
    </>
  )
}
